...
const GlobalTaskOverlay: React.FC = () => {
  ...
  const getTaskIcon = (type: string) => {
    switch (type) {
      case 'server_install': return <Server className="w-4 h-4" />;
      case 'plugin_install': return <Download className="w-4 h-4" />;
      case 'backup_create': return <Database className="w-4 h-4" />;
