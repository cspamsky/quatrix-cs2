...
const MonitoringSection: React.FC<MonitoringSectionProps> = ({ data, systemInfo, currentStats }) => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-[#1f2937] border border-gray-700 p-3 rounded-lg shadow-xl backdrop-blur-md min-w-[120px]">
          <p className="text-[10px] text-gray-400 font-bold mb-2 uppercase tracking-widest border-b border-gray-700 pb-1">{label}</p>
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xs font-black text-white uppercase tracking-wider">{t('dashboard.cpu_usage')}</h3>
                <p className="text-[9px] text-gray-500 font-bold truncate max-w-[120px]">{systemInfo?.cpuModel || 'Processor'}</p>
              </div>
            </div>
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xs font-black text-white uppercase tracking-wider">{t('dashboard.ram_usage')}</h3>
                <p className="text-[9px] text-gray-500 font-bold">{(parseFloat(currentStats.memUsed || '0')).toFixed(1)} / {((systemInfo?.totalMemory || 0) / 1024).toFixed(1)} GB</p>
              </div>
            </div>
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xs font-black text-white uppercase tracking-wider">{t('dashboard.net_traffic')}</h3>
                <div className="flex items-center gap-2 text-[9px] font-bold text-gray-500">
                  <span className="flex items-center gap-0.5"><ArrowDownLeft size={10} className="text-emerald-500" /> {currentStats.netIn}</span>
                  <span className="flex items-center gap-0.5"><ArrowUpRight size={10} className="text-blue-500" /> {currentStats.netOut}</span>
                </div>
              </div>
            </div>
            <div className="text-sm font-black text-emerald-500 mt-1 uppercase tracking-tighter">MB/s</div>
          </div>
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-xs font-black text-white uppercase tracking-wider">{t('dashboard.disk_io')}</h3>
                <p className="text-[9px] text-gray-500 font-bold uppercase">{currentStats.diskRead} READ / {currentStats.diskWrite} WRITE</p>
              </div>
            </div>
            <div className="text-sm font-black text-orange-500 mt-1 uppercase tracking-tighter">MB/s</div>
          </div>
