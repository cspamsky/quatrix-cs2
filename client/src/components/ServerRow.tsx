    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-3 mt-0.5">
           ...
           <span className="text-[10px] text-gray-500 font-mono">{serverIp}:{instance.port}</span>
        </div>
      </div>

      <div className="flex items-center gap-8 shrink-0 px-4">
        <div className="flex flex-col items-center min-w-[60px]">
          <span className="text-[9px] text-gray-600 font-black uppercase tracking-tighter mb-1">{t('serverCard.players')}</span>
          <div className="flex items-center gap-1.5">
            <Users size={12} className="text-gray-500" />
            <span className="text-xs font-bold text-gray-200">{instance.current_players} / {instance.max_players}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        {!instance.isInstalled ? (
           ...
           >
             <Download size={16} className={installingId === instance.id ? 'animate-bounce' : ''} />
           </button>
        ) : (
          <>
            {instance.status === 'OFFLINE' ? (
              ...
              <>
                ...
                >
                  <Square size={16} className={stoppingId === instance.id ? 'animate-pulse' : 'fill-current'} />
                </button>
                <button 
                ...
                >
                  <RotateCcw size={16} className={restartingId === instance.id ? 'animate-spin' : ''} />
                </button>
              </>
            )}
            <button 
