...
const Admins = () => {
  ...
  const admins = useMemo(() => {
    return Object.entries(adminsObj).map(([name, data]) => {
                      ...
                      <div className="flex flex-wrap gap-1">
                        {Array.isArray(admin.flags) ? (
                          admin.flags.map((flag: string) => (
                            <span
                              key={flag}
                              className="px-2 py-1 bg-gray-800 rounded-md text-[10px] text-gray-400 font-mono border border-gray-700"
                            >
                              {flag}
                            </span>
                          ))
