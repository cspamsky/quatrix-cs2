...
const Dashboard = () => {
  ...
  const { data: initialActivities } = useQuery<ActivityLog[]>({
  ...
  useEffect(() => {
    ...
    socket.on('dashboard_stats', (data: ServerDashboardStats) => {
      queryClient.setQueryData(['dashboard-stats'], data);
    });

    socket.on('recent_activity', (data: ActivityLog[]) => {
      setActivities(data);
    });

    socket.on('activity', (data: ActivityLog) => {
  ...
  const stats_items = [
    ...
    },
    {
      ...
      sub: `${t('dashboard.server_health')}: %${stats.healthScore || 100}`,
    },
    {
      label: t('dashboard.maps'),
      value: serverStats?.maps || 0,
      icon: MapIcon,
      color: 'text-orange-500',
      sub: t('dashboard.available_maps'),
    },
  ];
    ...
    <div className="w-full p-6 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats_items.map((item, index) => (
          <div
            key={index}
            className="p-6 bg-[#111827] rounded-2xl border border-gray-800/60 hover:border-white/10 transition-all hover:translate-y-[-2px] group shadow-lg shadow-black/20"
          >
      ...
      {/* Quick Actions Bar */}
      <div>
        <h2 className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-4 pl-1">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 font-medium mt-0.5">
                    Sistem ve yönetim günlüklerini detaylı inceleyin
                  </p>
                </div>
              </div>
              <button
              ...
              <div className="space-y-4">
                {filteredModalActivities.length > 0 ? (
                  filteredModalActivities.map((activity, idx) => (
                    <div
                      key={activity.id || idx}
                      className="p-4 bg-gray-800/20 border border-gray-800/40 rounded-2xl flex items-center justify-between hover:bg-white/[0.02] transition-all group"
                    >
                      <div className="flex items-center gap-5">
                        <div
                          className={`p-3 rounded-xl ${getSeverityColor(activity.severity)} shadow-lg shadow-black/20`}
                        >
                          {(() => {
                            const Icon = getActivityIcon(activity.type);
                            return <Icon size={18} />;
