...
const Profile = () => {
  ...
  const updatePasswordMutation = useMutation({
    mutationFn: (passwords: any) => apiFetch('/api/profile/password', {
      method: 'PUT',
  ...
  const updateAvatarMutation = useMutation({
    mutationFn: (url: string) => apiFetch('/api/profile/avatar', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const terminateSessionMutation = useMutation({
    mutationFn: (tokenId: string) => apiFetch(`/api/profile/sessions/${tokenId}`, { method: 'DELETE' }).then(res => res.json()),
    onSuccess: () => {
  ...
  const terminateAllSessionsMutation = useMutation({
    mutationFn: () => apiFetch('/api/profile/sessions', { method: 'DELETE' }).then(res => res.json()),
    onSuccess: () => {
  ...
  const setup2FAMutation = useMutation({
    mutationFn: () => apiFetch('/api/profile/2fa/setup', { method: 'POST' }).then(res => res.json()),
    onSuccess: (data) => {
      setSetupData(data)
  ...
  const verify2FAMutation = useMutation({
    mutationFn: (code: string) => apiFetch('/api/profile/2fa/verify', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const disable2FAMutation = useMutation({
    mutationFn: (password: string) => apiFetch('/api/profile/2fa/disable', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    <div className="p-6 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="bg-[#111827] border border-gray-800 rounded-2xl overflow-hidden shadow-xl">
            <div className="h-32 bg-gradient-to-r from-blue-600 to-indigo-700 relative">
              ...
              >
                ...
                />
                {user?.avatar_url ? (
                  <img src={getFullAvatarUrl(user.avatar_url)!} alt="Profile" className="w-full h-full object-cover group-hover:opacity-50 transition-opacity" />
                ) : (
            ...
            <div className="pt-16 pb-6 px-6">
              ...
              <div className="mt-8 space-y-4">
                ...
                <div className="flex items-center gap-3 text-sm text-gray-400">
                  <Clock size={16} />
                  <span>Joined: {user?.created_at ? new Date(user.created_at).toLocaleDateString() : 'Loading...'}</span>
                </div>
              </div>

              <div className="mt-8">
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 block">Avatar URL</label>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-[#111827] border border-gray-800 rounded-2xl p-8 shadow-xl">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className="text-xs text-gray-500 bg-gray-800/50 px-3 py-1 rounded-full">{sessions.length} Devices</span>
            </div>
            ...
            <div className="space-y-4">
              {sessions.length === 0 ? (
                ...
                <div key={session.id} className="group flex items-center gap-4 p-4 rounded-xl bg-[#0F172A]/50 border border-gray-800/50 hover:border-gray-700 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      ...
                      <span className="text-xs text-gray-500">Last active: {new Date(session.last_active).toLocaleString()}</span>
                    </div>
                  </div>
                  {user?.currentJti === session.token_id && (
                    <span className="text-[10px] font-bold bg-green-500/10 text-green-500 px-2 py-1 rounded-md border border-green-500/20 uppercase tracking-wider">Current Device</span>
                  )}
                  <button 
            ...
            <div className="mt-8 flex items-center gap-3 p-4 bg-red-400/5 border border-red-400/10 rounded-xl">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500">This will terminate all sessions except your current one.</p>
              </div>
              <button 
                onClick={() => {
      ...
      {show2FAModal && setupData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setShow2FAModal(false)}></div>
          <div className="relative bg-[#111827] border border-gray-800 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-8">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 block">Or enter code manually</label>
                  <div className="flex gap-2">
                ...
                <div>
                  <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 block">Verification Code</label>
                  <input 
                    type="text" 
                    maxLength={6}
                    value={twoFactorCode}
      ...
      {showDisable2FA && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-sm" onClick={() => setShowDisable2FA(false)}></div>
          <div className="relative bg-[#111827] border border-gray-800 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-8">
              <h3 className="text-xl font-bold text-white mb-2">Disable 2-Factor Authentication?</h3>
              <p className="text-sm text-gray-500 mb-6">Enter your password to confirm this action.</p>
