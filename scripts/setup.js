            ...
            try {
                ...
                if (userChoice.toLowerCase() !== 'n') {
                    try {
                        ...
                        log(C.magenta, "USER", "Creating user 'quatrix'...");
                        execSync('sudo useradd -m -s /bin/bash quatrix', { stdio: 'inherit' });
                        try {
                           execSync('sudo usermod -aG sudo quatrix', { stdio: 'inherit' }); 
                        } catch {}
                        log(C.green, "SUCCESS", "User 'quatrix' created.");
                    }
                    ...
                    log(C.magenta, "PERMS", "Fixing project permissions...");
                    const currentDir = process.cwd();
                    execSync(`sudo chown -R quatrix:quatrix ${currentDir}`, { stdio: 'inherit' });
                    try {
                        // Ensure critical scripts are executable
                        const scripts = ['server/src/index.ts', 'scripts/setup.js']; 
                        scripts.forEach(s => {
                             const p = path.join(currentDir, s);
                             if (fs.existsSync(p)) execSync(`sudo chmod +x ${p}`, { stdio: 'inherit' });
                        });
                    } catch {}
                    log(C.green, "SUCCESS", "Project ownership updated to quatrix:quatrix");
                }
...
WantedBy=multi-user.target`;
                    ...
                    fs.writeFileSync('/tmp/quatrix.service', serviceContent);
                    execSync('sudo mv /tmp/quatrix.service /etc/systemd/system/quatrix.service', { stdio: 'inherit' });
                    execSync('sudo systemctl daemon-reload', { stdio: 'inherit' });
                    execSync('sudo systemctl enable quatrix', { stdio: 'inherit' });
                    log(C.green, "SUCCESS", "Service installed & enabled. Start with 'sudo systemctl start quatrix'.");
                }
                ...
                if (fwChoice.toLowerCase() === 'y') {
                    ...
                    execSync('sudo ufw allow 3000/tcp', { stdio: 'inherit' });
