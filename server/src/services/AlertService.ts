...
class AlertService {
  ...
  public check(stats: SystemStats) {
    ...
    if (ramValue > this.thresholds.ram) {
      this.triggerAlert("CRITICAL_RAM", `Kritik Bellek Kullanımı: %${ramValue} (${stats.memUsed} GB / ${stats.memTotal} GB)`, "WARNING");
    }

    // Disk Kontrolü (Gelecekte si.fsSize() eklendiğinde genişletilebilir)
  }

  private triggerAlert(type: string, message: string, severity: "INFO" | "WARNING" | "ERROR" | "SUCCESS") {
    const now = Date.now();
